...
def main():
    ...
    for symbol, config in configs.items():
        dcm_pool[symbol] = dcm.start_futures_depth_cache(
            handle_depth_cache, 
            symbol=symbol, 
    ...
    while True:
        ...
        for symbol, dataframe in dataframes.items():
            ...
            if len(dataframe) > 300:
                    ...
                    file_name = f"data_futures/{symbol}/FUTURES-{symbol}-{start_time_dt}-{end_time_dt}.csv"
...
if __name__ == "__main__":
    ...
    dataframes = {}
    for symbol, config in configs.items():
        if not os.path.exists(f"data_futures/{symbol}"):
            os.makedirs(f"data_futures/{symbol}")
